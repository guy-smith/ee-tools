...
def main(ini_path, overwrite_flag=True):
    ...
    ini['EXPORT']['landsat_products'] = [
        'ndvi_sur', 'ndvi_toa', 'ndwi_toa', 'savi_sur', 'albedo_sur', 'ts', 'evi_sur', 'etstar_mean',
    ...
    header_list = [
        ...
        'NDVI_SUR', 'NDVI_TOA', 'NDWI_TOA', 'SAVI_SUR', 'ALBEDO_SUR', 'TS',
        'EVI_SUR', 'ETSTAR_MEAN',
        'ETG_MEAN', 'ETG_LPI', 'ETG_UPI', 'ETG_LCI', 'ETG_UCI',
            ...
            def beamer_zonal_stats_func(input_image):
                ...
                return ee.Feature(
                    None,
                    {
                        ...
                        'savi_sur': input_mean.get('savi_sur'),
            ...
            for ftr in stats_desc['features']:
                ...
                try:
                    row_list.append({
                        ...
                        'NDVI_SUR': float(ftr['properties']['ndvi_sur']),
                        'NDVI_TOA': float(ftr['properties']['ndvi_toa']),
                        'NDWI_TOA': float(ftr['properties']['ndwi_toa']),
                        'SAVI_SUR': float(ftr['properties']['savi_sur']),
...
def landsat_etg_func(img):
    ...
    ndvi_sur = img.select(['ndvi_sur'])
    #savi_sur = img.select(['savi_sur'])
    #etstar_mean = etstar_func(evi_sur, 'mean').rename(['etstar_mean'])
    etstar_mean = etstar_func(ndvi_sur, 'mean').rename(['etstar_mean'])
    #etstar_mean = etstar_func(savi_sur, 'mean').rename(['etstar_mean'])
    #etstar_lpi = etstar_func(evi_sur, 'lpi').rename(['etstar_lpi'])
    etstar_lpi = etstar_func(ndvi_sur, 'lpi').rename(['etstar_lpi'])
    #etstar_lpi = etstar_func(savi_sur, 'lpi').rename(['etstar_lpi'])
    #etstar_upi = etstar_func(evi_sur, 'upi').rename(['etstar_upi'])
    etstar_upi = etstar_func(ndvi_sur, 'upi').rename(['etstar_upi'])
    #etstar_upi = etstar_func(savi_sur, 'upi').rename(['etstar_upi'])
    #etstar_lci = etstar_func(evi_sur, 'lci').rename(['etstar_lci'])
    etstar_lci = etstar_func(ndvi_sur, 'lci').rename(['etstar_lci'])
    #etstar_lci = etstar_func(savi_sur, 'lci').rename(['etstar_lci'])
    #etstar_uci = etstar_func(evi_sur, 'uci').rename(['etstar_uci'])
    etstar_uci = etstar_func(ndvi_sur, 'uci').rename(['etstar_uci'])
    #etstar_uci = etstar_func(savi_sur, 'uci').rename(['etstar_uci'])
    ...
    return ee.Image([
            ...
            img.select(['ndvi_sur']), etstar_mean, etg_mean,
            img.select(['savi_sur']), etstar_mean, etg_mean,
            etg_lpi, etg_upi, etg_lci, etg_uci]) \
        .copyProperties(img, ['SCENE_ID', 'system:time_start'])


# the new evi_min (slightly above the x-intercept) is 0.04 for the
# 56 site-year regressions, respectively. Not 0.075 from the
...
# 56 site-year coeffs are being used for application to hydrographic areas.
